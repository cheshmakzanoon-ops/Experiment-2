...
enum class SelectionCombineMode(val displayName: String) {
    REPLACE("Replace"),
    ADD("Add"),
    SUBTRACT("Subtract"),
    INTERSECT("Intersect")
...
class ArtFlowCanvasView @JvmOverloads constructor(
    ...
    fun attachToCanvas(width: Int, height: Int, dpi: Int, backgroundColor: Int) {
    ...
    fun fitToView() {
        ...
        scale = (min(width.toFloat() / canvasWidth, height.toFloat() / canvasHeight) * 0.92f)
    ...
    fun canvasToView(x: Float, y: Float): Pair<Float, Float> {
    ...
    fun viewToCanvas(x: Float, y: Float): Pair<Float, Float> {
        val radians = Math.toRadians(rotationDegrees.toDouble())
        val cosR = cos(radians).toFloat()
        val sinR = sin(radians).toFloat()
    ...
    fun placeText(x: Float, y: Float, text: String, style: TextLayout.TextStyle, color: Int) {
    ...
    fun placeShape(
        ...
        coroutineScope.launch {
            val buffer = withContext(Dispatchers.Default) {
                buildShapeBuffer(
                    canvasWidth, canvasHeight, kind, startX, startY, endX, endY,
                    filled, color, strokeWidth
                )
            }
    ...
    fun moveActiveLayer(dx: Float, dy: Float) {
    ...
    fun transformActiveLayer(
        ...
        coroutineScope.launch {
            ...
            canvasRepository.applyRasterEdit(layerId, "Transform layer") { target ->
                target.clear()
                val dx = (pivotX - transformed.width / 2f).roundToInt() +
                    (canvasWidth - transformed.width) / 2
                val dy = (pivotY - transformed.height / 2f).roundToInt() +
    ...
    private fun continueGesture(event: MotionEvent, index: Int, x: Float, y: Float): Boolean {
        ...
        when (tool) {
            ToolType.BRUSH, ToolType.ERASER -> if (drawing) {
    ...
    private fun endGesture(
        ...
        val tool: ToolType = gestureTool ?: run {
        ...
        when (tool) {
            ...
            ToolType.MOVE, ToolType.TRANSFORM -> endPixelGesture(cancelled)
            ToolType.CLONE_STAMP -> {
    ...
    private fun startStroke(x: Float, y: Float, pressure: Float, tool: ToolType) {
        val params = if (tool == ToolType.ERASER) {
    ...
    private fun updateLiveStroke() {
        ...
        val copies = SymmetryEngine.mirrorStroke(stroke, canvasWidth, canvasHeight, input.symmetry)
     ...
     */
    private fun startPixelGesture(tool: ToolType, x: Float, y: Float, pressure: Float, description: String) {
        pixelTool = tool
    ...
    private fun initToolSession(tool: ToolType, buffer: PixelBuffer, x: Float, y: Float) {
        val selection = canvasRepository.selection()
        when (tool) {
            ToolType.SMUDGE -> smudgeSession = PixelBrushes.beginSmudge(
                x, y, input.smudge.copy(size = input.brushParams.size, mask = selection)
            )
            ToolType.CLONE_STAMP -> {
                val source = cloneSource ?: (x to y)
                cloneSession = PixelBrushes.beginCloneStamp(
                    targetX = x,
                    targetY = y,
            ...
            ToolType.LIQUIFY -> liquifySession = LiquifyTool.beginSession(
                x = x,
                y = y,
    ...
    private fun queuePixelSample(x: Float, y: Float, pressure: Float) {
    ...
    private fun applyPixelSample(x: Float, y: Float, pressure: Float) {
    ...
    private fun gradientFill(start: Pair<Float, Float>, end: Pair<Float, Float>) {
        val layerId = activeLayerId
        coroutineScope.launch {
            val selection = canvasRepository.selection()
            withContext(Dispatchers.Default) {
                canvasRepository.applyRasterEdit(layerId, "Gradient") { target ->
                    GradientTool.draw(
                        ...
                        endX = end.first,
                        endY = end.second,
                        settings = GradientTool.Settings(
    ...
    private fun commitSelectionDrag(tool: ToolType) {
        ...
        coroutineScope.launch {
            val mask = withContext(Dispatchers.Default) {
                when (tool) {
                    ToolType.SELECT_RECTANGLE -> SelectionMask.rectangle(
                        width, height,
                        points.first().first, points.first().second,
                        points.last().first, points.last().second
                    )
                    ToolType.SELECT_ELLIPSE -> SelectionMask.ellipse(
                        width, height,
                        points.first().first, points.first().second,
                        points.last().first, points.last().second
                    )
    ...
    private fun magicWandSelect(x: Float, y: Float) {
        val cx = x.roundToInt()
        val cy = y.roundToInt()
    ...
    private fun emitDragPreview(tool: ToolType, closed: Boolean = false) {
        val points = previewPoints.toList()
        if (points.isEmpty()) {
            onDragPreview?.invoke(null)
            return
    ...
    private fun applyBufferEdit(buffer: PixelBuffer, description: String) {
        val layerId = activeLayerId
        coroutineScope.launch {
            val selection = canvasRepository.selection()
    ...
    private fun drawShifted(target: PixelBuffer, source: PixelBuffer, dx: Int, dy: Int) {
    ...
    private fun buildShapeBuffer(
        ...
        color: Int,
        strokeWidth: Float
    ): PixelBuffer = BitmapPixelBridge.rasterizePath(
        width = width,
        height = height,
        build = { path ->
            when (kind) {
                ShapeKind.RECTANGLE -> path.addRect(
                    min(startX, endX), min(startY, endY),
                    max(startX, endX), max(startY, endY),
                    Path.Direction.CW
                )
                ShapeKind.ELLIPSE -> path.addOval(
    ...
    private fun snapped(event: MotionEvent, x: Float, y: Float, index: Int): Pair<Float, Float> {
        var (canvasX, canvasY) = viewToCanvas(x, y)
        if (!input.snapToGuides) return canvasX to canvasY

        if (input.symmetry.isActive()) {
            val snappedPoint = SymmetryEngine.snapToAxis(
                canvasX, canvasY, canvasWidth, canvasHeight, input.symmetry, SNAP_TOLERANCE
            )
            canvasX = snappedPoint.first
            canvasY = snappedPoint.second
        }
        if (input.perspective.isActive() && input.perspective.snapEnabled) {
            val snappedPoint = PerspectiveGuide.snap(
                canvasX, canvasY, input.perspective, canvasWidth, canvasHeight
            )
            canvasX = snappedPoint.first
            canvasY = snappedPoint.second
        }
    ...
    private fun orientationOf(event: MotionEvent, index: Int): Float {
        if (event.getToolType(index) != MotionEvent.TOOL_TYPE_STYLUS) return 0f
