...
sealed class LayerMaskEvent {
    data class MaskAdded(val mask: LayerMask, val creationMethod: MaskCreationMethod) : LayerMaskEvent()
    data class MaskRemoved(val layerId: Long) : LayerMaskEvent()
    data class MaskToggled(val layerId: Long, val isEnabled: Boolean) : LayerMaskEvent()
    data class MaskInverted(val layerId: Long, val isInverted: Boolean) : LayerMaskEvent()
    data class MaskDensityChanged(val layerId: Long, val density: Float) : LayerMaskEvent()
    data class MaskFeatherChanged(val layerId: Long, val featherRadius: Float) : LayerMaskEvent()
    data class MaskPainted(val layerId: Long, val affectedArea: MaskAffectedArea) : LayerMaskEvent()
    data class MaskLinked(val layerId: Long, val isLinked: Boolean) : LayerMaskEvent()
...
data class Rect(
    ...
    companion object {
        fun fromXYWH(x: Int, y: Int, width: Int, height: Int): Rect {
