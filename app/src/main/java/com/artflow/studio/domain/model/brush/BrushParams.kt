...
data class BrushParams(
     ...
     */
    fun calculateEffectiveSize(pressure: Float, velocity: Float = 0f): Float {
     ...
     */
    fun calculateEffectiveOpacity(pressure: Float, velocity: Float = 0f): Float {
     ...
     */
    private fun applyPressureCurve(pressure: Float, curve: PressureCurve): Float {
        return when (curve) {
     ...
     */
    fun applyColorJitter(baseColor: Int, pressure: Float = 1f, velocity: Float = 0f): Int {
        if (hueJitter <= 0f && saturationJitter <= 0f && brightnessJitter <= 0f && 
            !colorPressure && velocityToHue <= 0f) {
            return baseColor
        }
