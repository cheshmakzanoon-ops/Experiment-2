...
object PerspectiveGuide {
    ...
    enum class GuideType(val displayName: String) {
        NONE("None"),
        ONE_POINT("1-Point"),
        TWO_POINT("2-Point"),
        THREE_POINT("3-Point"),
     ...
     */
    data class VanishingPoint(val x: Float, val y: Float, val enabled: Boolean = true)
    ...
    data class Settings(
        ...
        fun activePointCount(): Int = when (type) {
    ...
    val DEFAULT_POINTS: List<VanishingPoint> = listOf(
    ...
    fun pointPosition(settings: Settings, index: Int, width: Int, height: Int): Pair<Float, Float> {
        val point = settings.vanishingPoints.getOrNull(index) ?: DEFAULT_POINTS[index.coerceIn(0, 2)]
        val y = if (index == settings.activePointCount() - 1 && settings.type == GuideType.THREE_POINT) {
    ...
    private fun isometricLines(settings: Settings, width: Int, height: Int): List<GuideLine> {
    ...
    fun withHorizon(settings: Settings, horizonY: Float): Settings =
        settings.copy(horizonY = horizonY.coerceIn(-1f, 2f))

    /** Moves vanishing point [index] to a normalised position. */
    fun movePoint(settings: Settings, index: Int, x: Float, y: Float): Settings {
    ...
    fun ensurePoints(settings: Settings, count: Int): Settings {
        val points = settings.vanishingPoints.toMutableList()
    ...
    fun nearestRayAngle(x: Float, y: Float, settings: Settings, width: Int, height: Int): Float? {
    ...
    val PRESETS: List<Preset> = listOf(
        ...
        Preset("Wide 2-Point", Settings(type = GuideType.TWO_POINT, vanishingPoints = listOf(VanishingPoint(-0.4f, 0.5f), VanishingPoint(1.4f, 0.5f)))),
    ...
    fun describe(settings: Settings): String = when (settings.type) {
    ...
    fun clipToCanvas(line: GuideLine, width: Int, height: Int): GuideLine? {
