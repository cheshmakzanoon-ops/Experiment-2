...
object ColorHarmony {
    ...
    enum class Harmony(val displayName: String, val description: String) {
     ...
     */
    fun harmony(baseColor: Int, harmony: Harmony, count: Int = 5): List<Int> {
        ...
        return when (harmony) {
            ...
            Harmony.TETRADIC -> listOf(
                baseColor,
                fromHsv(hue + 60f, saturation, value, alpha),
                fromHsv(hue + 180f, saturation, value, alpha),
                fromHsv(hue + 240f, saturation, value, alpha)
            )
            Harmony.SQUARE -> listOf(
                baseColor,
    ...
    fun fromRgb(red: Int, green: Int, blue: Int, alpha: Int = 255): Int =
    ...
    fun fromCmyk(cyan: Float, magenta: Float, yellow: Float, black: Float, alpha: Int = 255): Int {
     ...
     */
    val NAMED_COLORS: List<Pair<String, Int>> = listOf(
    ...
    fun isReadableOn(foreground: Int, background: Int, largeText: Boolean = false): Boolean =
        contrastRatio(foreground, background) >= if (largeText) 3f else 4.5f

    /** Black or white, whichever reads better on [background]. */
    fun bestTextColor(background: Int): Int =
        if (relativeLuminance(background) > 0.35f) 0xFF000000.toInt() else 0xFFFFFFFF.toInt()

    /** The classic colour-wheel position in degrees, for drawing a hue ring. */
    fun wheelPosition(hue: Float, radius: Float): Pair<Float, Float> {
    ...
    fun hueAt(dx: Float, dy: Float): Float {
    ...
    fun adjustLightness(argb: Int, amount: Float): Int {
        val hsv = Color.rgbToHsv(argb)
        val value = if (amount >= 0f) {
    ...
    fun tintsAndShades(argb: Int, steps: Int = 5): List<Int> {
    ...
    fun deduplicateRecent(colors: List<Int>, maxSize: Int = 24, threshold: Float = 6f): List<Int> {
        val result = mutableListOf<Int>()
    ...
    fun pushRecent(colors: List<Int>, color: Int, maxSize: Int = 24): List<Int> =
        deduplicateRecent(listOf(color) + colors, maxSize)

    /** All harmonies with their generated swatches, for the picker's harmony grid. */
    fun allHarmonies(baseColor: Int): Map<Harmony, List<Int>> =
        Harmony.entries.associateWith { harmony(baseColor, it) }
    ...
    fun formatChannels(argb: Int, mode: ColorMode): List<Pair<String, Float>> {
        val hsv = Color.rgbToHsv(argb)
        return when (mode) {
            ColorMode.HSV -> listOf(
                "H" to hsv[0],
                "S" to hsv[1],
                "V" to hsv[2]
            )
            ColorMode.RGB -> listOf(
    ...
    fun channelRange(mode: ColorMode, index: Int): ClosedFloatingPointRange<Float> = when (mode) {
        ColorMode.HSV -> when (index) {
            0 -> 0f..360f
            else -> 0f..1f
        }
    ...
    fun withChannel(base: Int, mode: ColorMode, index: Int, value: Float): Int {
