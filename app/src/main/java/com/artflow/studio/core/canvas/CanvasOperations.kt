...
object CanvasOperations {
    ...
    fun trimTransparent(
        ...
        val padded =
            content
                .inflated(padding.coerceAtLeast(0))
    ...
    fun fitToContent(
        buffer: PixelBuffer,
        properties: CanvasProperties,
    ): Result = trimTransparent(buffer, properties, padding = 0)
    ...
    fun changeDpi(
        properties: CanvasProperties,
        dpi: Int,
    ): CanvasProperties = properties.copy(dpi = dpi.coerceIn(MIN_DPI, MAX_DPI))
    ...
    fun isSizeSafe(
        width: Int,
        height: Int,
    ): Boolean = width in 1..MAX_DIMENSION && height in 1..MAX_DIMENSION && width.toLong() * height <= MAX_PIXELS
