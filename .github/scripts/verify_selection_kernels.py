#!/usr/bin/env python3
"""Run the real selection kernels without downloading Gradle or pretending to run Android.

Requires Python 3.10+, an installed Kotlin compiler, and Java 17+. This compiles the
unchanged production pixel sources and the same checks called by SelectionSafetyTest.
...
CHECKS = "app/src/test/java/com/artflow/studio/core/pixels/SelectionKernelChecks.kt"
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=root / "build/selection-kernel-checks")
    ...
    report = {
        "status": "not-run",
        "scope": "Standalone JVM selection kernels; NOT a Gradle, JUnit, Android, lint, signing, or release result",
        "heap_mb": args.heap_mb,
        "compiler": compiler,
        "java": java,
        "sha256": {name: hashlib.sha256((root / name).read_bytes()).hexdigest() for name in sources},
    }
    report_path = output / "selection-kernel-report.json"
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="artflow-selection-") as temporary:
            temp = Path(temporary)
            ...
            run_command = [
                ...
                "com.artflow.studio.core.pixels.SelectionKernelChecks",
            ]
            report["run_command"] = run_command
            result = execute(run_command, output / "results.txt", 45)
            match = re.search(r"^PASS selection-kernel-groups=(\d+) checks=(\d+)$", result, re.MULTILINE)
            if not match:
